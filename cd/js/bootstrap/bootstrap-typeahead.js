...
!function($) {
	...
	Typeahead.prototype = {
		...
		select : function() {
			var val = this.$menu.find('.active').attr('data-value'), res = this
					.getSelectedWord(this.$element.val())

			if (this.selectItems == 1) {
				this.$element.val(this.updater(val)).change()
			} else {
				res.length ? res += ';' + this.updater(val) : res += this
						.updater(val)
		...
		ajaxLookup : function() {
			...
			function execute() {
				...
				var params = this.ajax.preDispatch ? this.ajax
						.preDispatch(query) : {
					query : query
				}
				var jAjax = (this.ajax.method == "post") ? $.post : $.get;
			...
			this.ajax.timerId = setTimeout($.proxy(execute, this),
					this.ajax.timeout);
		...
		deleteWrongItems : function() {
			var that = this, map = {}, items = this.$element.val().split(';'), newArr = []
			// exist 2+ times
			if (items.length < 2) {
				if (items.length == 1) {
