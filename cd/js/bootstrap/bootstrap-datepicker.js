 ...
 * 
 * 
 * modified by jason @2012-09-30
 * =========================================================== */

!function($) {

	var selector = '[data-datepicker]', all = [];
	...
	function DatePicker(element, options) {
		this.$el = $(element);
		this.proxy('show').proxy('ahead').proxy('hide').proxy('keyHandler')
				.proxy('selectDate');
	...
	DatePicker.prototype = {
		...
		detectNative : function(el) {
			// Attempt to activate the native datepicker, if there is a known
			// good
			// one. If successful, return true. Note that input type="date"
			// requires that the string be RFC3339, so if the format/parse
			// methods
			// have been overridden, this won't be used.
			if (navigator.userAgent
					.match(/(iPod|iPad|iPhone); CPU(\ iPhone)? OS 5_\d/i)) {
		...
		init : function() {
			var $months = this.nav('months', 1), $years = this.nav('years', 12), $nav = $('<div>')
					.addClass('nav').append($months, $years), $calendar = $("<div>")
					.addClass('calendar'), i;

			// If the limit param is a Date obj, convert to an int offset in
			// milliseconds.
			...
			this.$picker = $('<div>').click(function(e) {
						e.stopPropagation();
					})
					// Use this to prevent accidental text selection.
					.mousedown(function(e) {
								e.preventDefault();
							}).addClass('datepicker').append($nav, $calendar)
					.insertAfter(this.$el);

			this.$el.focus(this.show).click(this.show).change($.proxy(
					function() {
						this.selectDate();
					}, this));
		...
		updateName : function($area, s) {
			...
			// animation.
			var cur = $area.find('.fg').text(), $fg = $('<div>').addClass('fg')
					.append(s), $bg = $('<div>').addClass('bg');
		...
		selectMonth : function(date) {
			var newMonth = new Date(date.getFullYear(), date.getMonth(), 1), rangeStart = this
					.rangeStart(date), rangeEnd = this.rangeEnd(date), num_days = this
					.daysBetween(rangeStart, rangeEnd), ii, today = new Date();
			...
			if (!this.curMonth
					|| !(this.curMonth.getFullYear() === newMonth.getFullYear() && this.curMonth
							.getMonth() === newMonth.getMonth())) {
				...
				for (ii = 0; ii <= num_days; ii++) {
					var thisDay = new Date(rangeStart.getFullYear(), rangeStart
									.getMonth(), rangeStart.getDate() + ii, 12,
							00), $day = $('<div>').attr('date',
							this.format(thisDay));
					...
					if (today.getTime() + this.limit < thisDay.getTime()
							|| (this.preventPast && today.getTime() > thisDay
									.getTime()))
				...
				$('div', this.$days).click($.proxy(function(e) {
					...
					// The date= attribute is used here to provide relatively
					// fast
					// selectors for setting certain date cells.
					this.update($targ.attr("date"));

					// Don't consider this selection final if we're just going
					// to an
					// adjacent month.
				...
				$("[date='" + this.format(today) + "']", this.$days)
						.addClass('today');

			}

			$('.selected', this.$days).removeClass('selected');
			$('[date="' + this.selectedDateStr + '"]', this.$days)
					.addClass('selected');
		},
		selectDate : function(date) {
			if (typeof(date) === "undefined") {
				var dataValue = this.$el.val(), arr = dataValue.split('-')
				
				if (arr.length == 3) {
					var month = parseInt(arr[1]), day = parseInt(arr[2])

					month > 12 ? arr[1] = '12' : month < 1
							? arr[1] = '01'
							: arr[1] < 10 ? arr[1] = '0' + month : ''

					var maxDay = new Date(arr[0], month, 0).getDate()
					day > maxDay ? arr[2] = '' + maxDay : day < 1
							? arr[2] = '01'
							: arr[2] < 10 ? arr[2] = '0' + day : ''
					
					this.$el.val(arr.join('-'))
					//console.log(arr, arr.join('-'))
					date = this.parse(arr.join('-'))
				}
			}
		...
		show : function(e) {
			...
			if (!this.noOffset) {
				...
				if (this.horizontal) {
					this.$picker.css({
								top : offset.top + this.$el.outerHeight() - 100,
								left : offset.left + this.$el.outerWidth() + 2
							});
				} else {
					this.$picker.css({
		...
		keyHandler : function(e) {
			...
			switch (e.keyCode) {
				case 9 :
				case 27 :
					// Tab or escape hides the datepicker. In this case, just
					// return
					// instead of breaking, so that the e doesn't get stopped.
					this.hide();
					return;
				case 13 :
					// Enter selects the currently highlighted date, unless it
					// is disabled.
					if ($('.selected', this.$days).hasClass('disabled'))
						return;
		...
		format : function(date) {
			...
			var month = (date.getMonth() + 1).toString(), dom = date.getDate()
					.toString();
			if (month.length === 1) {
				month = '0' + month;
			}
		...
		ahead : function(months, days) {
			// Move ahead ``months`` months and ``days`` days, both integers,
			// can be
			// negative.
			this.selectDate(new Date(this.selectedDate.getFullYear(),
					this.selectedDate.getMonth() + months, this.selectedDate
							.getDate()
							+ days));
		},
		proxy : function(meth) {
		...
		findClosest : function(dow, date, direction) {
			// From a starting date, find the first day ahead of behind it that
			// is
			// a given day of the week.
			var difference = direction
					* (Math.abs(date.getDay() - dow - (direction * 7)) % 7);
			return new Date(date.getFullYear(), date.getMonth(), date.getDate()
							+ difference);
		},
		rangeStart : function(date) {
			// Get the first day to show in the current calendar view.
			return this.findClosest(this.startOfWeek, new Date(date
									.getFullYear(), date.getMonth()), -1);
		},
		rangeEnd : function(date) {
			// Get the last day to show in the current calendar view.
			return this.findClosest((this.startOfWeek - 1) % 7, new Date(date
